...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # ROS parameters
        self.declare_parameter('can_port', 'can1')
        ...
        self.debug_joint_pub = self.create_publisher(JointState, 'joint_states_debug', 10)
    ...
    def PublishArmJointAndGripper(self):
        ...
        js.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, joint_6]

        # velocity & effort を必ずゼロ埋め
        js.velocity = [0.0] * len(js.position)
        js.effort   = [0.0] * len(js.position)

        # 元の joint_states_single に出す
        self.joint_pub.publish(js)

        # デバッグ用 joint_states_debug にも出す
        self.debug_joint_pub.publish(js)

        # 内部の self.joint_states も更新
    ...
    def PublishArmCtrlAndGripper(self):
        ...
        self.joint_ctrl.velocity = [0.0] * len(self.joint_ctrl.position)   # 追加
        self.joint_ctrl.effort = [0.0] * len(self.joint_ctrl.position)     # 追加
    ...
    def follow_joint_trajectory_goal(self, goal_request):
        """Goal受信時の処理"""
        self.get_logger().info("📥 Received new trajectory goal from MoveIt")
        return GoalResponse.ACCEPT  # 常に受け入れる

    def follow_joint_trajectory_cancel(self, goal_handle):
        """キャンセル要求時の処理"""
        self.get_logger().info("⚠️ Trajectory goal canceled by client")
        return CancelResponse.ACCEPT

    async def follow_joint_trajectory_execute(self, goal_handle):
        """MoveItから送られたFollowJointTrajectoryを実行"""
        self.get_logger().info("▶️ Executing FollowJointTrajectory goal")
